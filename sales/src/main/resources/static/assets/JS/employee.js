  ...
  dailyCallForm.addEventListener("submit", (e) => {
    ...
    if (totalCallsInput && successCallsInput) {
      ...
      if (success > total) {
        ...
        if (isValid) {
      const payload = {
        employeeName: document.getElementById("EmployeesName").value.trim(),
        totalCalls: document.getElementById("dailyCalls").value.trim(),
        successfulCalls: document.getElementById("dailySuccessCalls").value.trim(),
        avgDuration: document.getElementById("dailyDuration").value.trim(),
        ...
        leadsGenerate: document.getElementById("leadsGenerate").value,
        leadId: document.getElementById("lead_id").value.trim(),
        empId: document.getElementById("emp_id").value.trim(),
        leadDate: document.getElementById("lead_date").value,
        source: document.getElementById("source").value.trim(),
        convertedToDeal: document.getElementById("converted_to_deal").value,
        dealValue: document.getElementById("deal_value").value.trim()
      };
 
      const token = localStorage.getItem("authToken");
 
      fetch("http://localhost:9090/api/calls", {   // <-- CallDetailController POST endpoint
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        ...
        .then(data => {
          alert("Daily Call Detail Form submitted successfully!");
          dailyCallForm.reset();
        ...
        .catch(err => {
          console.error("Error saving daily call details:", err);
          alert("Failed to save daily call details: " + err.message);
        });
    }
